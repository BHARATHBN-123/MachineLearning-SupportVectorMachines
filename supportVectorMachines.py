        ...
        for i in range(n_samples):
            for j in range(n_samples):
                ...
                if self.kernel == 'polynomial':
                    K[i, j] = self.polynomial_kernel(
                        X[i], X[j], self.C, self.degree)
            ...
            for i in range(len(X)):
                ...
                for a, sv_y, sv in zip(self.alphas, self.sv_y, self.sv):
                    ...
                    if self.kernel == 'gaussian':
                        # Kernel trick.
                        s += a * sv_y * \
                            self.gaussian_kernel(X[i], sv, self.gamma)
                        self.C = None   # Not used in gaussian kernel.
                    if self.kernel == 'polynomial':
                        s += a * sv_y * \
                            self.polynomial_kernel(
                                X[i], sv, self.C, self.degree)
...
def formattedOutput(objFit):
    ...
    print(dash)
    print("               **** In-Sample: ****")
    print("{:>1} support vectors found from {:>1} examples.".format(
        len(objFit.sv), objFit.number_of_train_examples))
    print("               **** Predictions: ****")
    print("{:>1} out of {:>1} predictions correct.".format(
        objFit.number_of_test_examples, objFit.correct_predictions))
    print(dash)
...
def run_custom_svm(kernel='linear', C=0, gamma=0.1, degree=3):
    ...
    # Run Fit:
    # The amount of data assigned to training.
    percent = 0.80
    # Break up examples into training and test data.
    X_train, y_train, X_test, y_test = datageneration.Data.split_data(
        X1, y1, X2, y2, percent)
    # Fit the training data.
    objFit.fit(X_train, y_train)
    # Fit the out-of-sample (predicted data).
    y_predict = objFit.predict(X_test)
    # Check how many examples were predicted correctly.
    ...
    # Plot the margin and training data.
    objPlot.plot_margin(X_train[y_train == 1], X_train[y_train == -1], objFit)
...
def run_sklearn_svm(kernel='linear', C='auto', gamma='auto', degree='auto'):
    ...
    if kernel == 'linear':
        ...
        # Fit the training data.
        skl = SVC(kernel=kernel, coef0=C,  gamma=gamma,
                  degree=degree, tol=0.000001)
    elif kernel == 'rbf':
        X1, y1, X2, y2 = objData.gen_non_lin_separable_data(seed=1)
